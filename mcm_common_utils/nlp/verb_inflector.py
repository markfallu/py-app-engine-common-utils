...
def is_final_consonant_doubled(base):
    ...
    if TRACE:
        logger.debug('calling is_final_consonant_doubled...')

    # if the base form ends in 'c', double by adding 'k'
    if base.endswith('c'):
        if TRACE: logger.debug("\tends with 'c'")
    ...
    if match:
        if len(base) >= 4 and 'q' == base[-4] and 'u' == base[-3]:
            if TRACE: logger.debug('\tvvc ending with qu')
            # the qu acts like a qw, so really a q-consonant-vowel-consonant
            return base[-1]
        else:
            if TRACE: logger.debug('\tvvc ending, not qu-vowel-consonant')
            return []
    ...
    for phoneme_list in phoneme_lists:
        ...
        if match and not silent_t:
            if 1 == syllable_count:
                if TRACE: logger.debug('\tvc ending, single syllable, ' \
                                       'double final consonant')
                result = base[-1]
            elif is_final_syllable_stressed:
                if TRACE: logger.debug('\tvc ending, multi-syllable, ' \
...
def regular_simple_past(base):
    ...
    if TRACE:
        logger.debug("calling regular_simple_past for verb '{0}'".format(base))
    ...
    if base in EXCEPTIONS_PAST_TENSE:
        if TRACE: logger.debug('\tfound exception')
    ...
    if match:
        if TRACE: logger.debug('\tfound vowel-l ending')
        american = base + 'ed'
        british = base + 'led'
    ...
    if TRACE: logger.debug("\tadding default 'ed' ending")
    return [base + 'ed']
...
def present_participle(base):
    ...
    if TRACE:
        logger.debug("calling present_participle for verb '{0}'".format(base))
    ...
    if len(base) >= 2 and base.endswith('ie'):
        if TRACE: logger.debug('\treplacing -ie with -ying')
        return [base[:-2] + 'ying']

    # keep final 'e' if base form ends in 'ee', 'oe', or 'ye'
    match = regex_ee_oe_ye_ending.search(base)
    if match:
        if TRACE: logger.debug('\tends in ee, or, or ye')
        return [base + 'ing']
    ...
    try:
        ...
        in_cmu_dict = False
        if TRACE: logger.debug('\tnot in CMU dict')
        ...
        if in_cmu_dict:
            ...
            for phoneme_list in phoneme_lists:
                ...
                if silent_e and base in EXCEPTIONS_PP_SILENT_E:
                    # retain the final e
                    if TRACE: logger.debug('\tsilent-e exception')
                    results.add(base + 'ing')
                else:
                    # drop the final e
                    if TRACE: logger.debug('\tdropping final e')
    ...
    if TRACE: logger.debug('\tdefault -ing ending')
    return [base + 'ing']
...
def third_person_singular_present(base):
    ...
    if TRACE:
        logger.debug('calling third_person_singular_present...')
    ...
    if match:
        if TRACE: logger.debug('\tends in <consonant>o')
        return [base + 'es']
        ...
        for phoneme_list in phoneme_lists:
            ...
            if phoneme_list[-1] in SIBILANT_PHONEMES:
                ...
                if not base.endswith('e'):
                    if TRACE: logger.debug('\tends with sibilant sound, no silent-e')
                    ending = 'es'
            results.add(base + ending)
        return list(results)

    if TRACE: logger.debug("\tusing default 's' ending")
    return [base + 's']
...
def check_for_errors(base, truth_third, truth_pres_part,
    ...
    if has_error:
        ...
        logger.debug('\t    truth_third: {0}, computed: {1}'.format(truth_third, third))
        logger.debug('\ttruth_pres_part: {0}, computed: {1}'.format(truth_pres_part, pres_part))
        logger.debug('\t     truth_past: {0}, computed: {1}'.format(truth_past, past))
        logger.debug('\ttruth_past_part: {0}, computed: {1}'.format(truth_past_part, past_part))
