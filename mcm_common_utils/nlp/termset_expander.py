...
def log_token(token):
    ...
    logger.debug('[{0:3}]: {1:30}\t{2:6}\t{3:8}\t{4:12}\t{5}'.format(token.i,
                                                            token.text,
                                                            token.tag_,
                                                            token.pos_,
                                                            token.dep_,
...
def log_tokens(doc):
    ...
    logger.debug('\nTokens: ')
    logger.debug('{0:7}{1:30}\t{2:6}\t{3:8}\t{4:12}\t{5}'.format('INDEX', 'TOKEN', 'TAG',
                                                        'POS', 'DEP', 'HEAD'))
    for token in doc:
        log_token(token)
...
def expand(sentence, index_map):
    ...
    for index in indices[1:]:
        ...
        substitutions = index_map[index]
        #logger.debug('subs: {0}'.format(substitutions))
...
def get_synonyms(namespace, term_list, return_type=RETURN_TYPE_STRING):
    ...
    for t in term_list:
        ...
        if is_multiword:
            ...
            # do the expansion
            #logger.debug('index map: {0}'.format(index_map))
            new_phrases = expand(t, index_map)
            #logger.debug('new phrases: {0}'.format(new_phrases))
...
def expand_macros(str_termlist):
    ...
    for i in range(MAX_NESTED_DEPTH):
        ...
        for match in iterator:
            ...
            if macro_op is None:
                logger.debug('Unsupported macro: {0}'.format(matching_text))
                sys.exit(-1)
...
def run_tests():
    ...
    for verb, base in test_data.items():
        ...
        if trial != base:
            logger.debug('get_base_form: verb: {0}, expected base form {1}, got {2}'.
    ...
    logger.debug('Original: {0}'.format(sentence))
    for i in range(len(sentences)):
        logger.debug('\t[{0}]: {1}'.format(i, sentences[i]))
    ...
    for s in sentences:
        ...
        for i in range(len(new_phrases)):
            logger.debug('\t[{0}]: {1}'.format(i, new_phrases[i]))
